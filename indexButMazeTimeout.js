...
let newMazeLoading=false, newMazeTimeout=setTimeout(newMaze,9e4)
let newMazeStartDate=Date.now(), newMazeWinners=null, winnerDisplay=""
async function newMaze(){ //maze updated
    ...
    newMazeTimeout=setTimeout(newMaze,9e4);
        ...
        if(game.end[0]!==players[token].pos[0] || game.end[1]!==players[token].pos[1]){
            ...
            if(game.end[0]===players[token].pos[0] && game.end[1]===players[token].pos[1]){
                if(!newMazeWinners){
                    ...
                    newMazeTimeout=setTimeout(newMaze,Math.min(5e3,timeLeft))
                    newMazeWinners=[]
                }
